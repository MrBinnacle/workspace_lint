...
| Measurement | A counted fact or an observed timestamp about a resource the scan reached, published with a named unit, a link, and the set it was computed over. It makes no conformity claim, has no coverage item, enters no ratio, and reaches the exit byte through no channel. It is not a Rule: a rule tests one invariant and a count tests none. Every measurement carries a stable key, so it can later become a declared rule condition with no change to the measuring code. See ADR-0017. |
...
Eight distinctions the glossary enforces, because collapsing any of them breaks the product contract:
...
- **A measurement is not a finding.** A finding is a judgement a rule reached against an invariant. A measurement is a counted fact with no invariant behind it, so there is no value it could conform to. Collapsing them puts a count in a table a reader is entitled to read as defects, and it does so in the alarming direction — every rollup becomes something to fix. It also runs the other way: a rule ID attached to a count forces a coverage item and a conformity ratio for a claim that cannot be stated. This distinction was absent until ADR-0017, and the three signals it exists for were unbuilt for the whole of that time because nothing in the model had a place to put them.
...
`PRODUCT.md` holds the user, the job, the gates, and the kill criteria. This file is the glossary. Both are canonical. The research sweeps sit in `docs/research/`, indexed at `docs/research/INDEX.md` — start at the index, not the directory; the count is carried by the claim below rather than written out in prose, because a hand-kept scalar beside the fact it duplicates is what drifted on 2026-08-18. <!-- claim: count glob="docs/research/*.md" exclude="INDEX.md" equals=21 --> The Notion PRD that seeded this file is mirrored at `docs/inputs/prd-2026-08-16.md` and is an input, not an authority.
