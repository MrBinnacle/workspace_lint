...
Notion models it as **breaking inheritance**, not as a per-child deny. Two consequences. `unreached` inside a declared root **is** reachable by permissions, not only by rate limits and pagination — so ADR-0005's evidence-sufficiency axis keeps its full range and no superseding ADR is needed. And the action is admin-reversible, so a recorded gap is a fact about one scan window, which is what ADR-0002 decision 5 already implies by requiring start and end times.

**Still open, and it is the one that matters:** does `wl-revoke-parent` still list a `child_page` block for the disconnected child while a direct retrieve returns 404? That is an API observation the proof must make. The UI capability is confirmed; the API consequence is not.
